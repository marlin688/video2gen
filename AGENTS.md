...
- **B (Recordings/demos)** — User-provided screen captures, or screenshots auto-converted via `v2g record`. Missing recordings fall back to `terminal.aurora` style (animated Codex TUI simulation) in Remotion, or a placeholder card in FFmpeg.
...
- All others → Anthropic Codex SDK (streaming via httpx)
...
- `registry/` — Schema × Style component library system:
  ...
  - `styles/terminal/aurora.tsx` — Codex TUI simulation with aurora background (default for material B fallback)
...
- `ANTHROPIC_API_KEY` / `ANTHROPIC_BASE_URL` — Codex API (supports third-party gateways)
...
- `product`: Codex, Codex, cursor, github, vscode, chatgpt, openai, anthropic, google, deepseek, gemini, other
...
Rules for Codex when executing v2g pipeline stages:
